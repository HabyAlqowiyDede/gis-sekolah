...
class Sekolah extends BaseController
{
     ...
     */
    private function getSekolahViewPath(string $filename): string
    {
        return isSuperAdmin()
            ? "admin/superadmin/sekolah/{$filename}"
            : "admin/operator/sekolah/{$filename}";
    }

    /**
     * Upload beberapa foto galeri sekaligus (maks. 5 total per sekolah).
     */
    public function UploadGaleri()
    {
        ...
        $id_sekolah = $this->request->getPost('id_sekolah')
            ?? (isAdminSekolah() ? getCurrentUserSchoolId() : null);

        if (!$id_sekolah) {
            return $this->response->setJSON([
                'status'  => false,
                'message' => 'ID sekolah tidak tersedia',
            ]);
        }
        ...
        if (!(isSuperAdmin() || (isAdminSekolah() && getCurrentUserSchoolId() == $id_sekolah))) {
            return $this->response->setJSON([
                'status'  => false,
                'message' => 'Anda tidak memiliki izin untuk mengunggah foto untuk sekolah ini.',
            ]);
        }
        ...
        $fileList = $this->normalizeFileList($files);

        if (count($fileList) > 5) {
            return $this->response->setJSON([
                'status'  => false,
                'message' => 'Anda hanya dapat mengunggah maksimal 5 foto sekaligus.',
            ]);
        }
        ...
        if ($existingCount >= 5) {
            return $this->response->setJSON([
                'status'  => false,
                'message' => 'Kuota galeri sudah penuh. Hapus foto terlebih dahulu untuk mengunggah lagi.',
            ]);
        }
        ...
        if ($existingCount + count($fileList) > 5) {
            ...
            return $this->response->setJSON([
                'status'  => false,
                'message' => 'Anda hanya dapat mengunggah maksimal ' . $available . ' foto lagi.',
            ]);
        }
        ...
        return $this->response->setJSON([
            'status'  => false,
            'message' => 'Tidak ada file valid yang diunggah',
        ]);
    }

    /**
     * Normalisasi input file dari request menjadi array file tunggal
     * (mendukung input 'foto' tunggal maupun array/multiple).
     */
    private function normalizeFileList(array $files): array
    {
        ...
        // Fallback: ambil semua file yang ada
        $fileList = [];
        foreach ($files as $f) {
            if (is_array($f)) {
    ...
    public function UpdateData($id_sekolah)
    {
        ...
        // Field tambahan yang hanya wajib diisi oleh admin sekolah (bukan superadmin)
        if (!isSuperAdmin()) {
            ...
            $validationRules['banyak_guru'] = [
                ...
                'errors' => ['is_natural' => '{field} harus berupa angka'],
            ];
            $validationRules['visi'] = [
                'label'  => 'Visi',
                'rules'  => 'permit_empty|string',
                'errors' => ['string' => '{field} harus berupa teks'],
            ];
        ...
        if (!$this->validate($validationRules)) {
            ...
            return redirect()
                ->to(site_url('Sekolah/edit/' . $id_sekolah))
                ->withInput('validation', \Config\Services::validation()->getErrors());
        }
        ...
        if (isSuperAdmin()) {
            ...
            $data['banyak_guru']  = $this->request->getPost('banyak_guru')
                ? (int) $this->request->getPost('banyak_guru')
                : null;
    ...
    public function kecamatan()
    {
        ...
        foreach ($kecamatan as $value) {
            $selected = ($selectedIdKecamatan !== null && (string) $selectedIdKecamatan === (string) $value['id_kecamatan'])
                ? 'selected'
                : '';
    ...
    public function nagari()
    {
        ...
        foreach ($nagari as $value) {
            $selected = ($selectedIdNagari !== null && (string) $selectedIdNagari === (string) $value['id_nagari'])
                ? 'selected'
                : '';
