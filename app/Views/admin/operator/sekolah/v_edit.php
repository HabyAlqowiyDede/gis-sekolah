...
$nagari = $nagari ?? [];
$formData = $formData ?? [];
$formValue = function ($field, $default = '') use ($formData, $sekolah) {
    return $formData[$field] ?? $sekolah[$field] ?? $default;
};
...
<div class="col-md-12">
    ...
    <div class="card school-form-card">
        <div class="card-body">
            ...
            <?= form_open_multipart('Sekolah/UpdateData/' . $sekolah['id_sekolah'], ['id' => 'schoolForm']) ?>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="nama_sekolah" value="<?= esc($formValue('nama_sekolah')) ?>" placeholder="Contoh: SD Negeri 01" class="form-control">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>NPSN <span class="text-danger">*</span></label>
                            <input type="text" name="npsn" value="<?= esc($formValue('npsn', '')) ?>" placeholder="Contoh: 10100001" class="form-control">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="id_jenjang" class="form-control select2">
                                ...
                                <?php foreach ($jenjang as $jenjangItem): ?>
                                    <option value="<?= esc($jenjangItem['id_jenjang']) ?>" <?= ($jenjangItem['id_jenjang'] == ($formData['id_jenjang'] ?? $sekolah['id_jenjang'] ?? '')) ? 'selected' : '' ?>>
                                        <?= esc($jenjangItem['jenjang']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="id_kecamatan" id="id_kecamatan" class="form-control select2">
                                ...
                                <?php foreach ($kecamatan as $kecamatanItem): ?>
                                    <option value="<?= esc($kecamatanItem['id_kecamatan']) ?>" <?= ($kecamatanItem['id_kecamatan'] == ($formData['id_kecamatan'] ?? $sekolah['id_kecamatan'] ?? '')) ? 'selected' : '' ?>>
                                        <?= esc($kecamatanItem['nama_kecamatan']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
            ...
            <?php else: ?>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="id_kabupaten" id="id_kabupaten" class="form-control select2">
                                ...
                                <?php foreach ($kabupaten as $kabupatenItem): ?>
                                    <option value="<?= esc($kabupatenItem['id_kabupaten']) ?>" <?= ($kabupatenItem['id_kabupaten'] == ($formData['id_kabupaten'] ?? $sekolah['id_kabupaten'] ?? '')) ? 'selected' : '' ?>>
                                        <?= esc($kabupatenItem['nama_kabupaten']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control select2">
                                <option value="">Pilih</option>
                                <option value="Negeri" <?= ($formValue('status', '') === 'Negeri') ? 'selected' : '' ?>>Negeri</option>
                                <option value="Swasta" <?= ($formValue('status', '') === 'Swasta') ? 'selected' : '' ?>>Swasta</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <select name="akreditasi" class="form-control select2">
                                ...
                                <?php foreach (['A', 'B', 'C', 'D'] as $akreditasi): ?>
                                    <option value="<?= $akreditasi ?>" <?= ($formValue('akreditasi', '') === $akreditasi) ? 'selected' : '' ?>><?= $akreditasi ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="id_nagari" id="id_nagari" class="form-control select2">
                                ...
                                <?php foreach ($nagari as $nagariItem): ?>
                                    <option value="<?= esc($nagariItem['id_nagari']) ?>" <?= ($nagariItem['id_nagari'] == ($formData['id_nagari'] ?? $sekolah['id_nagari'] ?? '')) ? 'selected' : '' ?>>
                                        <?= esc($nagariItem['nama_nagari']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="alamat" id="alamat" class="form-control" rows="3" placeholder="Nama jalan, nomor rumah, atau deskripsi lokasi sekolah"><?= esc($formValue('alamat', '')) ?></textarea>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label>Visi</label>
                            <textarea name="visi" class="form-control" rows="3" placeholder="Visi sekolah"><?= esc($formValue('visi', '')) ?></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label>Misi</label>
                            <textarea name="misi" class="form-control" rows="4" placeholder="Misi sekolah (pisahkan dengan baris baru)"><?= esc($formValue('misi', '')) ?></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="kontak_admin" value="<?= esc($formValue('kontak_admin', '')) ?>" placeholder="Nomor telepon atau email" class="form-control" maxlength="50">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="banyak_guru" value="<?= esc($formValue('banyak_guru', '')) ?>" placeholder="Jumlah guru" class="form-control" min="0">
                        </div>
                    </div>
                </div>
                ...
                <input name="coordinat" id="Coordinat" value="<?= esc($formValue('coordinat', $sekolah['coordinat'] ?? '')) ?>" placeholder="Koordinat Sekolah (lat,lng)" class="form-control mt-2">
            <?php endif; ?>
...
<script>
    $(document).ready(function() {
        ...
        function saveDraft() {
            ...
            $.ajax({
                url: '<?= site_url('Sekolah/saveDraft/' . ($sekolah['id_sekolah'] ?? 0)) ?>',
                type: 'POST',
                data: draft,
                dataType: 'json'
            });
        ...
        function restoreDraft() {
            const savedDraft = localStorage.getItem(draftKey) || sessionStorage.getItem(draftKey);
            if (!savedDraft) {
                return;
            }
            ...
            try {
                ...
                Object.keys(draft).forEach(function(name) {
                    ...
                    if (field.is('select')) {
                        field.val(draft[name]).trigger('change');
                    } else if (field.attr('type') === 'radio' || field.attr('type') === 'checkbox') {
                        field.filter('[value="' + draft[name] + '"]').prop('checked', true);
                    } else {
                ...
                setTimeout(function() {
                    if (draft.alamat) {
                        $('textarea[name="alamat"]').val(draft.alamat);
                    }
                    if (draft.visi) {
                        $('textarea[name="visi"]').val(draft.visi);
                    }
                    if (draft.misi) {
                        $('textarea[name="misi"]').val(draft.misi);
                    }
                    if (draft.coordinat) {
                        $('#Coordinat').val(draft.coordinat);
                    }
                    $('.select2').trigger('change');
                }, 200);
            } catch (e) {
                console.log('Gagal memuat draft form', e);
            }
        }
        ...
        $('#id_kabupaten').on('change', function() {
            ...
            if (id_kabupaten) {
                $.ajax({
                    ...
                    data: { id_kabupaten: id_kabupaten, selected_id_kecamatan: $('#id_kecamatan').val() },
        ...
        function loadNagariByKecamatan(idKecamatan) {
            if (idKecamatan) {
                $.ajax({
                    ...
                    data: { id_kecamatan: idKecamatan, selected_id_nagari: $('#id_nagari').val() },
                    success: function(response) {
